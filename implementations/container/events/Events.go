...
import (
	...
	"github.com/simplecontainer/smr/pkg/reconciler"
...
func ListenDockerEvents(shared *shared.Shared) {
	...
	for {
		...
		case err := <-cErr:
			fmt.Println(err)
		case msg := <-cEvents:
			DockerEvent(shared, msg)
		}
	}
}

func ListenEvents(shared *shared.Shared, reconcilerObj reconciler.Reconciler) {
	for {
		select {
...
func Event(shared *shared.Shared, event reconciler.Events) {
...
func DockerEvent(shared *shared.Shared, event events.Message) {
	var container *container.Container
