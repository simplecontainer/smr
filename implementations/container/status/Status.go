...
func (status *Status) CreateGraph() {
	...
	status.StateMachine.AddEdge(start, prepareFailed)
	...
	status.StateMachine.AddEdge(prepareFailed, pendingDelete)
	status.StateMachine.AddEdge(prepareFailed, created)
