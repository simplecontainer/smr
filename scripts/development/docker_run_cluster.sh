...
export CLUSTER_DOMAIN_1="https://$(docker inspect -f '{{range.NetworkSettings.Networks}}{{.IPAddress}}{{end}}' smr-agent-1):9212"
sudo nohup smr node cluster start --node 1 --url $CLUSTER_DOMAIN_1  --cluster $CLUSTER_DOMAIN_1 &
...
sudo nohup smr node cluster start --node 2 --url $CLUSTER_DOMAIN_2 --cluster $CLUSTER_DOMAIN_1,$CLUSTER_DOMAIN_2 --join