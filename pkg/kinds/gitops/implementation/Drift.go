...
func (gitops *Gitops) Drift(client *client.Http, user *authentication.User, definitionsOrdered []FileKind) (bool, error) {
	for _, file := range definitionsOrdered {
		...
		response := network.Send(client.Clients[user.Username].Http, fmt.Sprintf("https://%s/api/v1/definition/compare", client.Clients[user.Username].API), http.MethodPost, bytes)
