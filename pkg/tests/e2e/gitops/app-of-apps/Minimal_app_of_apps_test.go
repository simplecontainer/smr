...
func TestStandaloneNodeAppOfAppsContainer(t *testing.T) {
	...
	nm.RunCommand(t, func() error {
		return cli.Smrctl.Run(t, engine.NewStringCmd("apply %s/%s/tests/gitops-app-of-apps",
	...
	go func() {
		nm.RunCommand(t, func() error {
			return cli.Smrctl.Run(t, engine.NewStringCmd("remove simplecontainer.io/v1/kind/gitops/examples/app-of-apps"))
		}, "remove container")
	}()
